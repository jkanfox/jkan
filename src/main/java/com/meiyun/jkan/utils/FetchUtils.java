...
public final class FetchUtils {
	...
	static final Logger logger = LoggerFactory.getLogger(FetchUtils.class);
	...
	public static WebModel connect(String url) {
		...
		try {
			...
			wm.setSuccess(false);
			logger.debug("自动抓取数据失败：{}", Throwables.getStackTraceAsString(e));
		}
		
		wm.setFetchTime(System.currentTimeMillis() - start);
		logger.info("网页数据：{}", JSON.toJSONString(wm));
		return wm;
	}
